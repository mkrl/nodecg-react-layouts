...
window.Bundle.people = (function () {
  const visibleRep = window.nodecg.Replicant('namesVisible')
  ...
  window.NodeCG.waitForReplicants(visibleRep)
    .then(() => {
  ...
  const peopleRep = window.nodecg.Replicant('people')
  let people
  const host1 = window.nodecg.Replicant('host1')
  const host2 = window.nodecg.Replicant('host2')
  const guest1 = window.nodecg.Replicant('guest1')
  const guest2 = window.nodecg.Replicant('guest2')
  const guest3 = window.nodecg.Replicant('guest3')
      ...
      data.forEach(item => {
        ...
        button.addEventListener('click', () => {
          if (safeToDelete(item.name) === true) {
            people = window.Bundle.common.removeFromArrayByProperty(people, 'name', item.name)
  ...
  const addNewPerson = () => {
    ...
    if (addName.value === '') {
      ...
      if (window.Bundle.common.isUniqueInArrayByProperty(people, 'name', newPerson.name)) {
